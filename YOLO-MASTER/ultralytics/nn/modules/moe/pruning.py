...
class MoEPruner:
    ...
    def _prune_experts(
        ...
        # Resize buffers to match new expert count
        new_count = len(keep_indices)
        if hasattr(moe_module, 'expert_usage_counts'):
            device = moe_module.expert_usage_counts.device
            moe_module.register_buffer(
                'expert_usage_counts', torch.zeros(new_count, device=device), persistent=False
            )
