...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif m.__name__ == 'ResizeTo':
            # ResizeTo는 입력 2개 [src, ref]를 받지만 출력 채널은 'src'의 채널을 유지
            # f가 [src_idx, ref_idx] 형태임을 가정
            assert isinstance(f, (list, tuple)) and len(f) == 2, "ResizeTo expects 2 inputs"
            c2 = ch[f[0]]
        else:
            c2 = ch[f]
